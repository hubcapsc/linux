					     ...
					     int *error_code)
{
	...
	if (ret > -1) {
		...
		*error_code = ret;
		...
		gossip_debug(GOSSIP_UTILS_DEBUG,
			     "pvfs2_create_dir: failed with error code %d\n",
			     *error_code);
	}
...
static inline struct inode *pvfs2_create_symlink(struct inode *dir,
						 struct dentry *dentry,
						 const char *symname,
						 int mode,
						 int *error_code)
{
	...
	if (ret > -1) {
		...
		/* finally, add dentry with this new inode to the dcache */
		gossip_debug(GOSSIP_UTILS_DEBUG,
			     "pvfs2_create_symlink: Instantiating\n"
			     "  *negative* dentry %p for %s\n",
			     dentry,
			     dentry->d_name.name);
		...
		*error_code = ret;
		...
		gossip_debug(GOSSIP_UTILS_DEBUG,
			    "pvfs2_create_symlink: failed with error code %d\n",
			    *error_code);
	}
