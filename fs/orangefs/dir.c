...
static int pvfs2_readdir(struct file *file, struct dir_context *ctx)
{
	 ...
	 * and ".." entries that are added below.
	 */
	new_op->upcall.req.readdir.token =
	    (pos == 0 ? PVFS_READDIR_START : pos);
...
get_new_buffer_index:
	...
	/*
	 * For the first time around, use the token returned by the
	 * readdir response
	 */
	if (token_set == 1) {
		...
		if (i == rhandle.readdir_response.pvfs_dirent_outcount) {
			...
			if (rhandle.readdir_response.token ==
			    PVFS_READDIR_END)
				/*
				 * If PVFS hit end of directory, then there
				 * is no way to do math on the token that it
				 * returned. Instead we go by the f_pos but
				 * back up to account for the artificial .
				 * and .. entries.  The fact that "token_set"
				 * is non zero indicates that we are on
				...
				file->f_pos = rhandle.readdir_response.token -
				    (rhandle.readdir_response.
				     pvfs_dirent_outcount - i + 1);
			gossip_debug(GOSSIP_DIR_DEBUG, "at least one dir_emit call failed. Setting f_pos to: %lld\n", lld(file->f_pos));
		}
	}

	/*
	 * end if token_set to 1. Did we hit the end of the directory?
	 */
	if (rhandle.readdir_response.token == PVFS_READDIR_END &&
	    !buffer_full) {
		gossip_debug(GOSSIP_DIR_DEBUG, "End of dir detected; setting f_pos to PVFS_READDIR_END.\n");
		file->f_pos = PVFS_READDIR_END;
	}

	gossip_debug(GOSSIP_DIR_DEBUG,
		     "pos = %llu, file->f_pos is %lld\n",
		     llu(pos),
		     lld(file->f_pos));
