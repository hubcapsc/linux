...
/*
 * the readdir response is a blank downcall (i.e. all these fields are
 * obtained from the trailer)
 */
typedef struct {
...
/*
 * the readdirplus response is a blank downcall (i.e. all these fields
 * are obtained from the trailer). Do not change the order of the fields.
 * Change it only if readdir_response_t is changing
 */
typedef struct {
	PVFS_ds_position token;
	uint64_t directory_version;
