...
struct rw_options {
	...
	const char *fnstr;
	ssize_t count;
	struct kiocb *iocb; /* Asynch I/O control block */
	union {
		struct {
			       ...
			       size_t total_actual_io)
{
	...
	if (amt_copied < total_actual_io) {
		gossip_err("copy_from_pagecache: user buffer size (%zd) is less than I/O size (%zd)\n", amt_copied, total_actual_io);
		kfree(copied_iovec);
		return -EINVAL;
	}
	index = 0;
	...
	while (amt_copied < total_actual_io) {
		...
		ret = copy_to_user(to_addr,
				   from_kaddr + page_offset,
				   cur_copy_size);
				  ...
				  size_t total_size)
{
	...
	if (!rw ||
	    ...
	    !rw->inode ||
	    !rw->fnstr) {
		gossip_lerr("invalid parameters (rw: %p, vec: %p, nr_segs: %lu, " "total_size: %zd)\n", rw, vec, nr_segs, total_size);
		ret = -EINVAL;
		goto out;
	}
...
populate_shared_memory:
	...
	gossip_debug(GOSSIP_FILE_DEBUG,
		     "%s/%s(%llu): GET op %p -> buffer_index %d\n",
		     __func__,
		     rw->fnstr,
	...
	gossip_debug(GOSSIP_FILE_DEBUG,
		     "%s/%s(%llu): copy_to_user %d nr_segs %lu, offset: %llu total_size: %zd\n", __func__, rw->fnstr, llu(rw->pvfs2_inode->refn.handle) , rw->copy_to_user_addresses, nr_segs, llu(*(rw->off.io.offset)) , total_size);
	...
	gossip_debug(GOSSIP_FILE_DEBUG,
		     "%s/%s(%llu): Calling post_io_request with tag (%llu)\n",
		     __func__,
		     rw->fnstr,
		     llu(rw->pvfs2_inode->refn.handle),
		     llu(new_op->tag));
	...
	gossip_debug(GOSSIP_FILE_DEBUG,
	    "%s/%s(%llu): Amount written as returned by the sys-io call:%d\n",
	    __func__,
	    rw->fnstr,
	    llu(rw->pvfs2_inode->refn.handle),
	...
	for (j = 0; j < page_idx; j++) {
		if (rw->dest.pages.pages[j]) {
			...
			if (rw->dest.pages.pg_byte_map[j]) {
				...
				/*
				 * if it was already cached, decrement its
				 * use count.
				 */
				page_cache_release(rw->dest.pages.pages[j]);
			}
			gossip_debug(GOSSIP_FILE_DEBUG,
				     "Releasing page %p (refcount %d)\n",
				     rw->dest.pages.pages[j],
				     pg_ref_count(rw->dest.pages.pages[j]));
		}
	}
...
static int check_mapping_tree(struct address_space *mapping, size_t file_size)
{
	...
	for (page_idx = 0; page_idx < nr_to_read; page_idx++) {
		...
		if (page) {
			gossip_debug(GOSSIP_FILE_DEBUG,
				     "check:(%ld) HIT page %p (refcount %d)"
				     "(page_offset %ld)\n", page_idx, page,
				     pg_ref_count(page), page_offset);
		} else {
			gossip_debug(GOSSIP_FILE_DEBUG,
				     "check: (%ld) MISS (page_offset %ld)\n",
...
static int locate_file_pages(struct rw_options *rw, size_t total_size)
{
	...
	x1 = isize - 1;
	x2 = offset + total_size - 1;
	min = PVFS_util_min(x1,x2) >> PAGE_CACHE_SHIFT;
	end_index = (unsigned long) min;
	gossip_debug(GOSSIP_FILE_DEBUG,
		     "filp: %p, inode: %p, mapping: %p\n",
		     rw->file,
		     rw->inode,
		     rw->inode->i_mapping);
	gossip_debug(GOSSIP_FILE_DEBUG,
		     "isize: %ld, offset (%ld) + total_size (%ld): %ld\n",
		     (long)isize,
		     (long)offset,
		     (long)total_size,
		     (long)offset + total_size);
	gossip_debug(GOSSIP_FILE_DEBUG,
		     "offset %lld, begin_index: %ld "
	...
	rw->dest.pages.pg_byte_map =
		kzalloc(nr_to_read * sizeof(*rw->dest.pages.pg_byte_map),
			PVFS2_BUFMAP_GFP_FLAGS);
	...
	rw->dest.pages.pages =
	    kzalloc(nr_to_read * sizeof(*rw->dest.pages.pages),
		    PVFS2_BUFMAP_GFP_FLAGS);
	...
	lock_mapping_tree(mapping);
	/*
	 * Preallocate all pages, increase their ref counts if they are
	 * in cache.
	 */
	for (page_idx = 0; page_idx < nr_to_read; page_idx++) {
		pgoff_t page_offset = begin_index + page_idx;
	...
	if (ret > 0) {
		/*
		 * Allocate memory for the pages against which I/O needs
		 * to be issued.
		 */
		rw->dest.pages.issue_pages =
			kzalloc(ret * sizeof(*rw->dest.pages.issue_pages),
				PVFS2_BUFMAP_GFP_FLAGS);
		if (!rw->dest.pages.issue_pages) {
			gossip_err("could not alloc memory for issue_pages\n");
			ret = -ENOMEM;
			goto cleanup;
		}
				  ...
				  size_t total_size)
{
	...
	gossip_debug(GOSSIP_FILE_DEBUG,
		     "total_size %zd, total # of pages %ld\n",
		     total_size,
	...
	gossip_debug(GOSSIP_FILE_DEBUG,
		     "total_actual_io to be staged from page-cache %zd\n",
		     total_actual_io);
	/*
	 * Copy the data from the page-cache to the application's
	 * address space
	 */
...
static ssize_t do_readv_writev(struct rw_options *rw)
{
	...
	gossip_debug(GOSSIP_FILE_DEBUG,
		"%s-BEGIN/%s(%llu): count(%d) after estimate_max_iovecs.\n",
		__func__,
		rw->fnstr,
	...
	if (rw->type == IO_WRITEV) {
		...
		gossip_debug(GOSSIP_FILE_DEBUG,
			     "%s/%s(%llu): proceeding with offset : %llu, "
			     "size %d\n",
			     __func__,
			     rw->fnstr,
			     llu(pvfs2_inode->refn.handle),
	...
	gossip_debug(GOSSIP_FILE_DEBUG,
		     ...
		     llu(*offset));
	gossip_debug(GOSSIP_FILE_DEBUG,
		     "%s/%s(%llu): new_nr_segs: %lu, seg_count: %lu\n",
		     __func__,
		     rw->fnstr,
		     llu(pvfs2_inode->refn.handle),
...
/* PVFS2_KERNEL_DEBUG is a CFLAGS define. */
	...
	while (total_count < count) {
		...
		gossip_debug(GOSSIP_FILE_DEBUG,
			     "%s/%s(%llu): size of each_count(%d)\n",
			     __func__,
			     rw->fnstr,
			     llu(pvfs2_inode->refn.handle),
			     (int)each_count);
		gossip_debug(GOSSIP_FILE_DEBUG,
			     "%s/%s(%llu): BEFORE wait_for_io: offset is %d\n",
			     __func__,
			     rw->fnstr,
			     llu(pvfs2_inode->refn.handle),
			     (int)*offset);
		...
		gossip_debug(GOSSIP_FILE_DEBUG,
			     "%s%s(%llu): return from wait_for_io:%d\n",
			     __func__,
			     rw->fnstr,
			     llu(pvfs2_inode->refn.handle),
			     (int)ret);
		...
		*offset += ret;
		...
		gossip_debug(GOSSIP_FILE_DEBUG,
			     "%s/%s(%llu): AFTER wait_for_io: offset is %d\n",
			     __func__,
			     rw->fnstr,
			     llu(pvfs2_inode->refn.handle),
			     (int)*offset);
 ...
 * Return value for this routine is negative in case of errors
			...
			unsigned long **seg_array)		/* OUT */
{
	...
	unsigned long count;
	unsigned long begin_seg;
	unsigned long tmpnew_nr_segs;
	struct xtvec *new_xtvec = NULL;
	struct xtvec *orig_xtvec;
	unsigned long *sizes = NULL;
	unsigned long sizes_count = 0;
	...
	*seg_array = NULL;
	...
	orig_xtvec = kmalloc(nr_segs * sizeof(*orig_xtvec),
			     PVFS2_BUFMAP_GFP_FLAGS);
	if (orig_xtvec == NULL) {
		gossip_err
		    ("split_xtvecs: Could not allocate memory for %lu bytes!\n",
...
repeat:
	for (seg = begin_seg; seg < nr_segs; seg++) {
		if (tmpnew_nr_segs >= max_new_nr_segs ||
			...
			kfree(new_xtvec);
			gossip_err
			    ("split_xtvecs: exceeded the index limit (%lu)\n",
			     tmpnew_nr_segs);
			return -EINVAL;
		}
		if (count + orig_xtvec[seg].xtv_len <
		    pvfs_bufmap_size_query()) {
			count += orig_xtvec[seg].xtv_len;

			memcpy(&new_xtvec[tmpnew_nr_segs],
			       &orig_xtvec[seg],
			       sizeof(*new_xtvec));
			tmpnew_nr_segs++;
			sizes[sizes_count]++;
		} else {
			new_xtvec[tmpnew_nr_segs].xtv_off =
			    orig_xtvec[seg].xtv_off;
...
static long bound_max_xtvecs(const struct xtvec *curr,
			     unsigned long nr_segs,
			     size_t * total_count)
{
	unsigned long i;
			    ...
			    size_t total_size)
{
	...
	if (!rw->pvfs2_inode ||
	    !rw->inode ||
	    !rw->fnstr) {
		gossip_lerr("invalid parameters: pvfs2_inode: %p, inode: %p, "
			    "fnstr: %p\n",
			    rw->pvfs2_inode,
			    rw->inode,
			    rw->fnstr);
	...
	if ((ret = precopy_buffers(buffer_index,
				   rw,
				   vec,
				   nr_segs,
				   total_size)) < 0)
	...
	if (ret < 0) {
		...
		if (ret == -EINTR)
			...
			gossip_err("%s: error in %s handle %llu, "
				   "FILE: %s\n  -- returning %ld\n",
				   rw->fnstr,
				   rw->type == IO_READX ?
					"noncontig read from" :
	...
	if ((ret = postcopy_buffers(buffer_index,
			    rw,
			    vec,
			    nr_segs,
...
static ssize_t do_readx_writex(struct rw_options *rw)
{
	...
	unsigned long xtnr_segs;
	struct xtvec *xtvec;
	unsigned long max_new_nr_segs_mem;
	unsigned long max_new_nr_segs_stream;
	unsigned long new_nr_segs_mem = 0;
	unsigned long new_nr_segs_stream = 0;
	unsigned long seg_count_mem;
	unsigned long *seg_array_mem = NULL;
	unsigned long seg_count_stream;
	unsigned long *seg_array_stream = NULL;
	struct iovec *iovecptr = NULL;
	struct iovec *ptr = NULL;
	struct xtvec *xtvecptr = NULL;
	struct xtvec *xptr = NULL;
	...
	/*
	 * Compute total and max number of segments after split of the
	 * memory vector
	 */
	if ((max_new_nr_segs_mem =
	     bound_max_iovecs(iov, nr_segs, &count_mem)) < 0) {
		gossip_lerr("%s: could not bound iovec %lu\n",
			    rw->fnstr,
			    max_new_nr_segs_mem);
		goto out;
	...
	/*
	 * Calculate the total stream length amd max segments after split
	 * of the stream vector
	 */
	if ((max_new_nr_segs_stream =
	     bound_max_xtvecs(xtvec, xtnr_segs, &count_stream)) < 0) {
	...
	if (count_mem > pvfs_bufmap_size_query()) {
		...
		ret = split_iovecs(max_new_nr_segs_mem,	/* IN */
				   ...
				   &seg_array_mem);	/* OUT */
		if (ret < 0) {
			gossip_err("%s: Failed to split iovecs to satisfy larger than blocksize readx request %ld\n", rw->fnstr, (long)ret);
			goto out;
		}
		/*
		 * We must free seg_array_mem and iovecptr, xtvecptr and
		 * seg_array_stream
		 */
		to_free = 1;
		gossip_debug(GOSSIP_FILE_DEBUG,
			     "%s: Splitting iovecs from %lu "
			     "to %lu [max_new %lu]\n",
			     rw->fnstr,
			     nr_segs,
			     new_nr_segs_mem,
		...
		if (ret < 0) {
			gossip_err("Failed to split iovecs to satisfy larger than blocksize readx request %ld\n", (long)ret);
			goto out;
		}
		gossip_debug(GOSSIP_FILE_DEBUG,
			     "%s: Splitting xtvecs from %lu to "
			     "%lu [max_new %lu]\n",
			     rw->fnstr,
			     xtnr_segs,
			     new_nr_segs_stream,
...
/* PVFS2_KERNEL_DEBUG is a CFLAGS define. */
	...
	for (seg = 0; seg < new_nr_segs_stream; seg++)
		gossip_debug(GOSSIP_FILE_DEBUG,
			     "%s: %d) %ld to %ld [%ld bytes]\n",
			     rw->fnstr,
			     seg + 1,
			     (long)xtvecptr[seg].xtv_off,
			     (long)xtvecptr[seg].xtv_off +
				 xtvecptr[seg].xtv_len,
	...
	while (total_count < count_mem) {
		...
		/* how much to transfer in this loop iteration */
		each_count =
		    (((count_mem - total_count) > pvfs_bufmap_size_query()) ?
			pvfs_bufmap_size_query() :
			(count_mem - total_count));
		/* and push the I/O directly through to the servers */
		ret = wait_for_iox(rw,
				   ptr,
				   seg_array_mem[seg],
				   xptr,
				   seg_array_stream[seg],
				   each_count);
		if (ret < 0)
			goto out;
		/* Advance the iovec pointer */
...
static ssize_t pvfs2_file_readx(struct file *file,
				const struct iovec *iov,
				unsigned long nr_segs,
...
static ssize_t pvfs2_file_writex(struct file *file,
				 const struct iovec *iov,
				 unsigned long nr_segs,
				 const struct xtvec *xtvec,
				 unsigned long xtnr_segs)
...
/* RESET_FILE_POS is a configure option: --enable-reset-file-pos */
	...
	if ((x = (pvfs2_kiocb *) iocb->private) == NULL) {
		...
		if (rw->type == IO_WRITE) {
			 ...
			 */
			if (rw->copy_to_user_addresses)
			  error = pvfs_bufmap_copy_iovec_from_user(buffer_index,
					iov,
					nr_segs,
					count);
			else
			  error =
			    pvfs_bufmap_copy_iovec_from_kernel(buffer_index,
					iov,
					nr_segs,
					count);
			if (error < 0) {
