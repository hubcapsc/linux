			       ...
			       size_t total_actual_io)
{
	...
	gossip_debug(GOSSIP_FILE_DEBUG, "copy_from_pagecache: nr_segs %ld, total_actual_io: %zd, total pages %ld\n",
		     nr_segs,
		     total_actual_io,
				  ...
				  size_t total_size)
{
	...
	err = locate_file_pages(&rw, total_size);
	if (err < 0) {
