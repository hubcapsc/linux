...
static int bufmap_page_count;
static int pages_per_desc;

static int32_t pvfs2_bufmap_desc_size;
static int32_t pvfs2_bufmap_desc_shift;
...
int pvfs_bufmap_initialize(struct PVFS_dev_map_desc *user_desc)
{
	...
	if (user_desc->total_size != (user_desc->size * user_desc->count)) {
		gossip_err("pvfs2 error: user provided an oddly sized buffer: ("
			   "%d, %d, %d)\n",
			   user_desc->total_size,
			   user_desc->size,
			   user_desc->count);
	...
	if ((user_desc->size % PAGE_SIZE) != 0) {
		gossip_err("pvfs2 error: bufmap size not page size divisible ("
			   "%d).\n",
			   user_desc->size);
		goto init_failure;
	}
	...
	bufmap_page_array =
	    kmalloc(bufmap_page_count * sizeof(*bufmap_page_array),
		    PVFS2_BUFMAP_GFP_FLAGS);
			       ...
			       size_t size)
{
	...
	if (bufmap_init == 0) {
		gossip_err
		    ("pvfs_bufmap_copy_from_user: not yet initialized.\n");
		gossip_err
		    ("pvfs2: please confirm that pvfs2-client daemon is running.\n");
		up_read(&bufmap_init_sem);
		return -EIO;
			      ...
			      size_t size)
{
	...
	gossip_debug(GOSSIP_BUFMAP_DEBUG,
		     "pvfs_bufmap_copy_to_pages: nr_pages %lu,"
		     "index %d, size %zd\n",
		     nr_segs,
		     buffer_index,
		     size);
	...
	down_read(&bufmap_init_sem);
	if (bufmap_init == 0) {
		gossip_err("pvfs_bufmap_copy_to_pages: not yet initialized.\n");
		gossip_err
		    ("pvfs2: please confirm that pvfs2-client is running.\n");
		up_read(&bufmap_init_sem);
		return -EIO;
	}
	...
	while (amt_copied < size) {
		if (from_page_index >= nr_segs) {
			gossip_err
			    ("pvfs_bufmap_copy_to_pages: count cannot exceed number of pages(%lu)\n", nr_segs);
			up_read(&bufmap_init_sem);
			return -EIO;
		}
		...
		if (page == NULL) {
			gossip_err
			    ("pvfs_bufmap_copy_to_pages: invalid page pointer "
			    "%d\n",
			     from_page_index);
			up_read(&bufmap_init_sem);
			return -EIO;
		...
		cur_copy_size = ((amt_remaining > PAGE_SIZE) ?
				  ...
				  amt_remaining);
		gossip_debug(GOSSIP_BUFMAP_DEBUG,
			     "pvfs_bufmap_copy_to_pages: from_page: "
			     "%p, to_page: %p\n",
			     from->page_array[from_page_index],
			     page);
		...
		if (cur_copy_size < PAGE_SIZE)
			memset(to_kaddr + cur_copy_size,
			       0,
			       PAGE_SIZE - cur_copy_size);
				...
				size_t size)
{
	...
	gossip_debug(GOSSIP_BUFMAP_DEBUG,
		     "pvfs_bufmap_copy_from_pages: nr_pages %lu "
		     "index %d, size %zd\n",
		     nr_segs,
		     buffer_index,
		     size);
	...
	if (bufmap_init == 0) {
		gossip_err
		    ("pvfs_bufmap_copy_from_pages: not yet initialized.\n");
		gossip_err
		    ("pvfs2: please confirm that pvfs2-client is running.\n");
		up_read(&bufmap_init_sem);
		return -EIO;
	...
	while (amt_copied < size) {
		if (to_page_index >= nr_segs) {
			gossip_err
			    ("pvfs_bufmap_copy_from_pages: count cannot exceed number of pages(%lu)\n", nr_segs);
			up_read(&bufmap_init_sem);
			return -EIO;
		}
		...
		if (page == NULL) {
			gossip_err
			    ("pvfs_bufmap_copy_from_pages: invalid page pointer\n");
			up_read(&bufmap_init_sem);
			return -EIO;
		}
		amt_remaining = (size - amt_copied);
		cur_copy_size = ((amt_remaining > PAGE_SIZE) ?
				  ...
				  amt_remaining);
		gossip_debug(GOSSIP_BUFMAP_DEBUG,
			     "pvfs_bufmap_copy_from_pages: from_page: "
			     "%p, to_page: %p\n",
			     page,
			     to->page_array[to_page_index]);
		to_kaddr = pvfs2_kmap(to->page_array[to_page_index]);
				       ...
				       size_t size)
{
	...
	if (copied_iovec == NULL) {
		gossip_err
		    ("pvfs2_bufmap_copy_iovec_from_kernel: failed allocating memory\n");
		up_read(&bufmap_init_sem);
		return -ENOMEM;
	}
	memcpy(copied_iovec, iov, nr_segs * sizeof(*copied_iovec));
	...
	for (seg = 0, amt_copied = 0; seg < nr_segs; seg++)
		amt_copied += copied_iovec[seg].iov_len;
	if (amt_copied != size) {
		gossip_err("pvfs2_bufmap_copy_iovec_from_kernel: computed total"
			   "(%zd) is not equal to (%zd)\n",
			   amt_copied,
			   size);
		kfree(copied_iovec);
				   ...
				   size_t size)
{
	...
	for (seg = 0, amt_copied = 0; seg < nr_segs; seg++)
		amt_copied += copied_iovec[seg].iov_len;
	if (amt_copied < size) {
		gossip_err("pvfs2_bufmap_copy_to_user_iovec: computed total "
			   "(%zd) is less than (%zd)\n",
			   amt_copied,
			   size);
		kfree(copied_iovec);
				     ...
				     size_t size)
{
	...
	if (copied_iovec == NULL) {
		gossip_err
		    ("pvfs2_bufmap_copy_to_kernel_iovec: failed allocating memory\n");
		up_read(&bufmap_init_sem);
		return -ENOMEM;
	}
	memcpy(copied_iovec, iov, nr_segs * sizeof(*copied_iovec));
	...
	if (amt_copied < size) {
		gossip_err
		    ("pvfs2_bufmap_copy_to_kernel_iovec: computed total (%zd) is less than (%zd)\n",
		     amt_copied,
		     size);
		kfree(copied_iovec);
		up_read(&bufmap_init_sem);
					   ...
					   size_t size_to_be_copied)
{
	...
	if (copied_iovec == NULL) {
		gossip_err
		    ("pvfs_bufmap_copy_to_user_iovec: failed allocating memory\n");
		up_read(&bufmap_init_sem);
		return -ENOMEM;
	}
	...
	if (amt_copied < size_to_be_copied) {
		gossip_err("pvfs_bufmap_copy_to_user_task_iovec: computed total"
			   " (%zd) is less than (%zd)\n",
			   amt_copied,
			   size_to_be_copied);
		kfree(copied_iovec);
	...
	while (amt_copied < size_to_be_copied) {
		...
		if (iv->iov_len < (PAGE_SIZE - from_page_offset)) {
			cur_copy_size =
			    PVFS_util_min(iv->iov_len,
					  size_to_be_copied - amt_copied);
			seg++;
		...
		if (copy_amt != 0)
			gossip_err("%s: failure in copy_to_user, "
